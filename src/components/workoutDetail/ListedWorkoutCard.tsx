...
//           {/* ডিউরেশন, ক্যালরি এবং রেটিং তথ্য */}
...
//           className="text-gray-500 hover:text-gray-300 text-sm font-bold pl-1 transition-colors cursor-pointer"
//         >
//           ✕
//         </button>
//       </div>
...
import { WorkoutContext, WorkoutContextType } from '@/context/WorkoutContext'; // আপনার context-এর সঠিক path অনুযায়ী পরিবর্তন করুন
import { IWorkout } from '@/types/workoutType';
import Image from 'next/image';
import Link from 'next/link';
...
const ListedWorkoutCard = ({ workout }: { workout: IWorkout }) => {
  ...
  // Context থেকে setPlan নিয়ে আসা
  // const context = useContext(WorkoutContext);
  // const { setPlan, setSaved } = (context || {}) as WorkoutContextType;
  const {setPlan} = useContext(WorkoutContext) as WorkoutContextType;

  // Item রিমুভ করার হ্যান্ডলার ফাংশন
  const handleDeleteWorkout = (targetId: number) => {
    if (setPlan) {
      setPlan((prevPlan) => prevPlan.filter((item) => item.id !== targetId));
    }
